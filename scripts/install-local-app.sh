...
# Không dùng `| grep -q` ở đây: grep thoát sớm, codesign nhận SIGPIPE, và với
# `pipefail` cả điều kiện thành sai dù chữ ký đúng (dính đúng vậy lần cài
# 0.1.2 đầu tiên, 15/09).
signature="$(codesign -dvv "$dst" 2>&1 || true)"
