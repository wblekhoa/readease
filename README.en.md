...
### [⬇️ Download ReadEase 0.1.13 for Apple Silicon Macs - 132 MB](https://github.com/wblekhoa/readease/releases/download/v0.1.13/ReadEase-0.1.13-arm64.dmg)

One click downloads the `.dmg`. Open it and **drag `ReadEase.app` into Applications** right there. No Terminal, no tools to install. Prefer a `.zip`? [Here](https://github.com/wblekhoa/readease/releases/download/v0.1.13/ReadEase-0.1.13-arm64.zip) (115 MB) - unzip and drag the same way. From this version the app tells you when a newer one exists (ReadEase menu › Check for Updates…). Other versions and release notes: [Releases](https://github.com/wblekhoa/readease/releases).

### Pick by what you want to do

There is **one** app; the voices are chosen inside it, and the app **downloads only what you pick**. Add the sizes up to know what you will download:

| You want to | Get | Total | Where |
| --- | --- | --- | --- |
| Read **Vietnamese**, small | [App](https://github.com/wblekhoa/readease/releases/download/v0.1.13/ReadEase-0.1.13-arm64.zip) + Vietnamese voice *Standard* | 115 MB + 330 MB | First screen "Choose how to read, to begin" → Vietnamese → **Download and use** on the *Standard · 330 MB* row |
| Read **Vietnamese**, best quality | [App](https://github.com/wblekhoa/readease/releases/download/v0.1.13/ReadEase-0.1.13-arm64.zip) + Vietnamese voice *Highest* | 115 MB + 625 MB | Same screen → **Download and use** on the *Highest · 625 MB* row (reads ~11 % slower) |
| Read **English** | [App](https://github.com/wblekhoa/readease/releases/download/v0.1.13/ReadEase-0.1.13-arm64.zip) + English voice Kokoro (6 American voices) | 115 MB + 330 MB | First screen (or **Voices & models** on the home screen) → English → **Download** |
| Read **both languages** | [App](https://github.com/wblekhoa/readease/releases/download/v0.1.13/ReadEase-0.1.13-arm64.zip) + Vietnamese + English voices | 115 MB + 660-955 MB | Both steps above; any model can be removed later |
| Use **paid AI voices** (OpenAI / ElevenLabs), no model download | [App](https://github.com/wblekhoa/readease/releases/download/v0.1.13/ReadEase-0.1.13-arm64.zip) + your API key | 115 MB | **Voices & models → API voices → Add key**; reads both languages, billed per character |
...
The first screen, **"Choose how to read, to begin"**, lists what this Mac can read with: one group per language, one row per voice model with its size and a **Download and use** button. Nothing is required - **Go to the library** is always available, and voices can be added later under **Voices & models** on the home screen. Only what you pick is downloaded: Vietnamese *Standard* ~330 MB, *Highest* ~625 MB (reads about 11 % slower), English ~330 MB; reading is offline after that. Switching models mid-reading asks first and says how much would be downloaded; a model you no longer use is removed with one button in the same place.
...
The English option translates the app interface, status messages and common import errors. Switching the UI language does not install the English speech model - that is a separate, optional download under Voices & models.
