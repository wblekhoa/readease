//! The Python reading engine as a child process: its stdin and stdout, the
//! JSONL protocol, and the client the commands call.
//!
//! The shell stays thin on purpose: it forwards JSONL requests, turns `chunk`
//! events into PCM frames for the audio thread (`audio.rs`), and reports
//! state to the webview. Everything that knows Vietnamese - prosody, voices,
//! caching - lives on the other side of the pipe. Audio lives in the host
//! rather than in the webview because reading usually starts from a global
//! shortcut, exactly where WKWebView's autoplay policy would mute a web
//! AudioContext. The speaker itself - the bounded queue, the lookahead, the
//! positions announced when the ear reaches them, the replay after a break -
//! is `audio.rs`; this file hands it frames and reads its answers.
...
mod tests {
    ...
    use super::*;
    use crate::audio::{drain, AUDIO_QUEUE_FRAMES, PLAYER_LOOKAHEAD, PREBUFFER_MAX_WAIT, SAMPLE_RATE};
