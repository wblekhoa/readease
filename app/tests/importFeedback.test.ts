...
test("kéo qua cửa sổ: nói số sách sẽ thêm, hoặc rằng không có sách nào", () => {
  setLanguage("vi");
...
test("một tệp: hai câu quen thuộc; nhiều tệp: đếm; có lỗi: nói lỗi", () => {
  ...
  assert.deepEqual(importNotice({ added: 2, existing: 0, failed: 1, lastError: "PDF không có lớp văn bản." }),
    { tone: "error", message: "Đã thêm 2 sách; 1 tệp không nhập được. PDF không có lớp văn bản." });
});
